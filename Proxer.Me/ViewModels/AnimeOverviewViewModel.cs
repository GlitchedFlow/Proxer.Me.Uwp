...
{
	public class AnimeOverviewViewModel : ViewModel<AnimeOverviewModel>
	{
		static AnimeOverviewViewModel()
		{
			Instance = new AnimeOverviewViewModel();
		}

		public static AnimeOverviewViewModel Instance { get; private set; }
		...
		public AnimeOverviewViewModel() : base(new AnimeOverviewModel())
		{
		...
		private async void ExecuteRefreshList(object obj)
		{
			...
			switch (Model.SelectedType)
			{
				...
				case 2:
					...
					break;
				case 3:
					medium = Medium.Hentai;
					break;
				default:
					medium = Medium.AnimeSeries;
					break;
			}
			var list = await ListAPI.GetEntryList(false, medium, false, start, Model.CurrentPage, 26);
			if (list.Error)
			{
			...
			else
			{
				...
				foreach (var anime in list.Data)
				{
					if (list.Data.Count == 26 && list.Data.IndexOf(anime) == 25)
					{
						continue;
					}
					Model.Data.Add(new EntryListRecord(anime));
				}
			}
			Model.IsWorking = false;
			RefreshList.RaiseCanExecuteChanged();
